...
$select = $obj->select('*', 'student_info');
...
<div class="container mt-5">
  <h2 class="mb-4 ml-5">School Data</h2>
    ...
    if (!empty($select)) {
      foreach ($select as $row) { ?>
        ...
        <tr>
          ...
          <td><?= $row['block'] ?></td>
          <td><?= $row['school'] ?></td>
          <td><?= $row['std_images'] ?></td>
          <td><a  class="btn btn-primary" href='student_collection.php?id=<?= $row['id'] ?>'>Show</a></td>
          <td><?= $row['remarks'] ?></td>
